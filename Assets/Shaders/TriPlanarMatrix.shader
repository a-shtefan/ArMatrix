...
#define dropLength 512
         float3 rain(float2 fragCoord)
         {
           ...
           uint _session_rand_seed; // required by the RandomLib Include
...
#include "LabColorspace.cginc"
         ...
         float3 MatrixEffect(float2 coord) {
             ...
             float3 rain_col = rain(coord* float2(dropLength, dropLength)*_Scale);

             if (global_colored == 1)
                 rain_col = rain_colored(coord * float2(dropLength, dropLength)*_Scale);
             return                 text(coord * float2(dropLength, dropLength)*_Scale) *rain_col;
         }

         int       _Global_Shader_On;
         // -----------------------------------
         fixed4 frag (v2f i) : SV_Target
         {
             ...
             // Evaluate the weights from the normal, increase sharpness for better look
             float3 blendWeight  = pow(normalize(abs(i.normal)), _Sharpness);
                    blendWeight /= (blendWeight.x + blendWeight.y + blendWeight.z);

            // Blend the projections into weighted average
            ...
            // The external turn on/off button
                    col      *= _Global_Shader_On;
                    col       = min(1.5,col);
             return col;
         }
         ENDCG
